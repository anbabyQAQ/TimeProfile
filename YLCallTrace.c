    ...
    if (mainThreadStack && mainThreadStack->index >= 0) {
        ...
        if (costTime >= costMinTime) {
            if (++mainThreadCallRecord->index >= mainThreadCallRecord->allocLength) {
                ...
                mainThreadCallRecord->record = realloc(mainThreadCallRecord->record, mainThreadCallRecord->allocLength * sizeof(YLCallRecord));
            }
            YLCallRecord* callRecord = &mainThreadCallRecord->record[mainThreadCallRecord->index];
...
void initData()
{
    if (!mainThreadCallRecord) {
        ...
        mainThreadCallRecord->allocLength = 128;
        mainThreadCallRecord->record = (YLCallRecord *)malloc(mainThreadCallRecord->allocLength * sizeof(YLCallRecord));
