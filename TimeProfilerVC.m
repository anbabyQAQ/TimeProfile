...
#import "YLCallTrace.h"
#import "YLRecordCell.h"
...
- (void)setRecordDic:(NSMutableArray *)arr record:(YLCallRecord *)record
{
    if ([arr isKindOfClass:NSMutableArray.class] && record) {
        int total=1;
        for (NSUInteger i = 0; i < arr.count; i++)
        {
            YLRecordModel *model = arr[i];
        ...
        YLRecordModel *model = [[YLRecordModel alloc] initWithCls:record->cls sel:record->sel time:record->costTime depth:record->depth total:total];
...
- (void)stopAndGetCallRecord
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        while (i <= mainThreadCallRecord->index) {
            ...
            {
                YLCallRecord *callRecord = &mainThreadCallRecord->record[j];
                ...
                if (callRecord->depth==0 || j==mainThreadCallRecord->index)
                {
                    ...
                    YLRecordHierarchyModel *model = [[YLRecordHierarchyModel alloc] initWithRecordModelArr:recordModelArr];
...
- (NSString *)debug_getMethodCallStr:(YLCallRecord *)callRecord
{
...
- (void)sortCostTimeRecord:(NSArray *)arr
{
    NSArray *sortArr = [arr sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        YLRecordHierarchyModel *model1 = (YLRecordHierarchyModel *)obj1;
        YLRecordHierarchyModel *model2 = (YLRecordHierarchyModel *)obj2;
        if (model1.rootMethod.costTime > model2.rootMethod.costTime) {
            return NSOrderedAscending;
        }
        return NSOrderedDescending;
    }];
    for (YLRecordHierarchyModel *model in sortArr) {
        model.isExpand = NO;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)arrAddRecord:(YLRecordModel *)model arr:(NSMutableArray *)arr
{
    for (int i = 0; i < arr.count; i++) {
        YLRecordModel *temp = arr[i];
...
- (void)sortCallCountRecord:(NSArray *)arr
{
    ...
    NSArray *sortArr = [arrM sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        YLRecordModel *model1 = (YLRecordModel *)obj1;
        YLRecordModel *model2 = (YLRecordModel *)obj2;
...
{
    if (self.tableType == tableTypeSequential) {
        YLRecordHierarchyModel *model = self.sequentialMethodRecord[section];
    ...
    {
        YLRecordHierarchyModel *model = self.costTimeSortMethodRecord[section];
        if (model.isExpand && [model.subMethods isKindOfClass:NSArray.class]) {
            return model.subMethods.count+1;
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    YLRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:YSRecordCell_reuseIdentifier];
    if (!cell) {
        cell = [[YLRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:YSRecordCell_reuseIdentifier];
    }
    YLRecordHierarchyModel *model;
    YLRecordModel *recordModel;
